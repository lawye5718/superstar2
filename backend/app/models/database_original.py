...
def get_uuid_type(as_uuid=False):
    ...
    # Check database type from settings, but also allow override via environment
    db_url = getattr(settings, 'DATABASE_URL', '')
    # Check if we're running tests with SQLite
    import os
    test_db_url = os.environ.get('DATABASE_URL', os.environ.get('SQLALCHEMY_DATABASE_URL', ''))
    
    if 'sqlite' in test_db_url.lower() or 'sqlite' in db_url.lower():
        # SQLite doesn't have UUID type, use String instead
        return String(36)
    elif "postgresql" in db_url:
        return PostgresUUID(as_uuid=as_uuid)
    else:
        # For other databases, use String(36) as fallback
        return String(36)
